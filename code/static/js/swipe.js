...
HiddenGems.swipe = {
     ...
     * @private
     */
    // Update in swipe.js - _changeActiveGem function
    _changeActiveGem: function(newIndex) {
        // Update the map controller's active gem
        if (HiddenGems.map) {
            HiddenGems.map.activeGemIndex = newIndex;
        }

        // Update map marker/popup
        if (typeof window.highlightGemMarker === 'function') {
            window.highlightGemMarker(newIndex);
        } else if (HiddenGems.map && typeof HiddenGems.map.showGemPopup === 'function') {
